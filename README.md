# MFB Tanzbär 2 Sample Manager App

![Tests](https://linuxbender.github.io/tanzbaer-2-app/badge-tests.svg)

Browser-based sample upload tool for the MFB Tanzbär 2 drum machine.

🌐 **[Open App](https://linuxbender.github.io/tanzbaer-2-app/)**

---

## Requirements

- Chrome or Edge (Web MIDI required)
- MFB Tanzbär 2 connected via USB or MIDI

---

## Disclaimer

This app is provided as-is, without any warranty or guarantee that it works correctly.
Use at your own risk.

---

## License

© 2026 — [CC BY-NC-ND 4.0](./LICENSE)
Free to use, no commercial use, no redistribution as your own product.
